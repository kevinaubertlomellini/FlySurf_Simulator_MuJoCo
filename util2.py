...
def init_vectors2(n_actuators, n_points, iter, n_points_sampled, N_horizon ):
    ...
    return [u_save, x_save, xd_save, xe_save,step_time_save, x_gamma_save, u_components_save, xd_sampled, t_save]

def plot_errors(iter, delta, xd_save, xe_save, x_gamma, n_tasks):
...
def plot_positions(t, x_save, xd_save):
    ...
    for i in range(4):
        axes[0].plot(t[:positions_x.shape[1]], positions_x[i, :], color=cmap(0), label=f"x_{i + 1}")
        axes[0].plot(t[:positions_d_x.shape[1]], positions_d_x[i, :], '--', color=cmap(0),  label=f"xd_{i + 1}",alpha=0.3)
        axes[1].plot(t[:positions_x.shape[1]], positions_y[i, :], color=cmap(1), label=f"y_{i + 1}")
        axes[1].plot(t[:positions_d_x.shape[1]], positions_d_y[i, :], '--', color=cmap(1), label=f"yd_{i + 1}", alpha=0.3)
        axes[2].plot(t[:positions_x.shape[1]], positions_z[i, :], color=cmap(2), label=f"z_{i + 1}")
        axes[2].plot(t[:positions_d_x.shape[1]], positions_d_z[i, :], '--', color=cmap(2), label=f"zd_{i + 1}", alpha=0.3)
