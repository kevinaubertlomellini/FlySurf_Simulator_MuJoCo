...
[u_save, x_save, xd_save, xe_save, step_time_save, x_gamma_save, u_components_save, xd_sampled] = init_vectors2(n_actuators, [rows, cols], iter, [n_points, n_points2], N_horizon )
...
# CONTROL PARAMETERS
Q_vector = [1500, 1500, 8, 8] # [x and y, z, velocity in x and y, velocity in z]
...
mpc = init_MPC_model4(x, str_stif,shear_stif,flex_stif,damp_point,damp_quad,l0,n_points, n_points2, n_actuators, x_actuators, mass_points*rows*cols/(n_points*n_points2), mass_quads,Q_vector, R_vector, delta, u_limits, g, xd_sampled, N_horizon)
mpc.setup()
mpc.x0 = x
...
plot_errors(iter+1, delta, x_save, xd_save, xe_save, x_gamma_save, n_tasks)
...
ax.text(iter * 0.7, np.max(step_time_save) * 1.02, f"Mean: {np.mean(step_time_save):.2f}", color='k', fontsize=12)
