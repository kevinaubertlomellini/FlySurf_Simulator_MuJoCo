...
rows2 = 13 # Number of rows (n-1)/(spacing+1)
...
u_limits = np.array([[-0.8, 0.8], [-0.8, 0.8], [-5.0, 10.0]]) # Actuator limits
...
# CONTROL PARAMETERS
Q_vector = [500, 500, 16000000, 0, 0, 5, 5, 8000, 8000, 8000] # [x, y, z, v_x and v_y, v_z, x_UAV and y_UAV, z_UAV , v_x_quad, v_y_quad, v_z_quad]
R_vector = [30, 30, 30] # [force in x and y, force in z]
...
factor= 0.2
shape_gaussian = shape_gaussian_mesh(sides=[0.9, 0.9], amplitude=1.0, center=[0.0, 0.0], sd = [0.575, 0.575], n_points = [rows, cols])
...
flysurf = CatenaryFlySurf(rows2, cols2, 1/(rows2-1) + 0.001, num_sample_per_curve=rows)
...
for ii in range(iter+N_horizon+1):
    ...
    if ii<=iter:
        ...
        combined2 = np.hstack((xd_pos_vector[indices2], 0*u_shape_vector[indices2]))
