...
alpha_G = 0
alpha_Rs = 0.65*np.array([900,900,1000])
Q_0 = 190000 * np.eye(6)
Q_0[0:2, 0:2] = 38000 * np.eye(2)
Q_0[3:5, 3:5] = 0 * np.eye(2)
Q_0[5, 5] = 0
R_vector = 150*np.array([25, 0.8*105])
...
with (mujoco.viewer.launch_passive(model, data) as viewer):
    ...
    output = cv2.VideoWriter("SMPC_Mujoco_9UAV.avi", cv2.VideoWriter_fourcc(*'MPEG'), 1 / T_s,
    ...
    while viewer.is_running() and data.time <= total_time:
        ...
        if time_num%delta_factor == 0:
            ...
            u = u_mpc + 1.03*u_gravity  # Compute control inputs for all drones
