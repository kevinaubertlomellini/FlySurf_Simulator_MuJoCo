...
spring_factor = 50  # HERE

alpha_H = 0.8*np.array([45,45,60])
alpha_G = 0
alpha_Rs = 0.65*np.array([900,900,1000])
Q_0 = 190000 * np.eye(6)
Q_0[0:2, 0:2] = 42000 * np.eye(2)
Q_0[3:5, 3:5] = 0 * np.eye(2)
Q_0[5, 5] = 0
R_vector = 100*np.array([20, 105])
...
with (mujoco.viewer.launch_passive(model, data) as viewer):
    ...
    while viewer.is_running() and data.time <= total_time:
        ...
        if time_num%delta_factor == 0:
            ...
            u = u_mpc + 1.023*u_gravity  # Compute control inputs for all drones
