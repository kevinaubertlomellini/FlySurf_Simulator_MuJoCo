...
def generate_xml_space(rows, cols, x_init, y_init, x_length, y_length, quad_positions, mass_points, mass_quads, str_stif,
                 shear_stif, flex_stif, damp_point, damp_quad, delta, u_limits, max_l_str, max_l_shear, max_l_flex, gravity, file_path):
    """
    Generates XML for a grid of rows and columns with specified spacing and saves it to a file.
  ...
  <asset>
    <!-- Define the STL mesh -->
    <mesh name="drone_body" file="tugnew4.stl" scale="0.0018 0.0018 0.0018"/>
   
    <!-- Optional material -->
    <material name="drone_material" rgba="0.75 0.75 0.75 1"/>
  </asset>
  ...
  <asset>
    <!-- Define the texture -->
    <texture name="earth_texture" type="2d" file="earth2.png" />

    <!-- Define the material that uses the texture -->
    <material name="earth_material" texture="earth_texture" texuniform="false"/>
  </asset>


  <worldbody>
    <light directional="true" pos="0 0 3" castshadow="false"/>
    <geom name="earthp"  pos="-2 25 -43" size="32" type="sphere" material="earth_material" contype="0" conaffinity="0"/>
""")
    ...
    for row in range(rows):
        for col in range(cols):
            ...
            if [row + 1, col + 1] in quad_positions_ordered:  # Positions are 1-indexed
                body_template = f"""
    <body name="quad_{element_counter}" pos="{x_pos:.4f} {y_pos:.4f} 0.5" childclass="x2">
        <joint type="free" damping="{damp_quad}"/>
        <geom material="drone_material" mesh="drone_body" class="visual"  pos="0.05 -0.045 0" quat="0 0 1 1"/>
        <geom class="collision" size=".009 .00405 .003" pos=".006 0 .003"/>
        <geom class="collision" size=".009 .00405 .003" pos=".006 0 .009"/>
        <geom class="collision" size=".0075 .00405 .003" pos="-.0105 0 .00975"/>
        <geom class="collision" size=".00345 .00255 .0015" pos="-.02055 .0012 .00975" quat="1 0 0 1"/>
    ...
    <motor class="x2" name="thrust{element_counter}_z" site="ball_{row + 1}_{col + 1}" gear="0 0 1 0 0 0" ctrlrange="{u_limits[2, 0]} {u_limits[2, 1]}"/>""")
                ...
                body_template = f"""
    <body pos="{x_pos:.4f} {y_pos:.4f} 0.5">
        <joint type="free" damping="{damp_point}"/> 
        <geom class="ball"/>
        <site name="ball_{row + 1}_{col + 1}" pos="0 0 0"/>
    </body>
