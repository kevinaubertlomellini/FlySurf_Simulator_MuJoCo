...
# CONTROL PARAMETERS
Q_vector = [20, 20, 20, 10] # [x and y, z, velocity in x and y, velocity in z]
R_vector = [2, 10] # [force in x and y, force in z]
...
mpc = init_MPC_model(x, str_stif,shear_stif,flex_stif,damp_point,damp_quad,l0,n_points, n_points2, n_actuators, x_actuators, mass_points, mass_quads,Q_vector, R_vector, delta, u_limits)
...
with mujoco.viewer.launch_passive(model, data) as viewer:
    ...
    while viewer.is_running() and data.time <= total_time:
        ...
        if time_num%delta_factor ==0:
            ...
            data.ctrl[:] = u.flatten()
            u_save[:, int(time_num/delta_factor)] = u.flatten()
