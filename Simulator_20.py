...
mpc_Rs = init_MPC_Rs_shape(str_stif,shear_stif,flex_stif,damp_point,damp_quad,l0,n_points, n_points2, n_actuators, x_actuators, mass_total/(n_points*n_points2), mass_quads,Q_vector, R_vector, delta, u_limits, g, Rs_d_save, shape_save , xd_0_save ,N_horizon)
mpc_Rs.setup()
mpc_Rs.x0 = x
...
with mujoco.viewer.launch_passive(model, data) as viewer:
    ...
    while viewer.is_running() and data.time <= total_time:
        ...
        if time_num%delta_factor == 0:
            ...
            u_mpc_0 = 35*mpc_0.make_step(x)

            u_mpc_Rs = 60*mpc_Rs.make_step(x)

            u_mpc = u_mpc_0 + u_mpc_Rs
...
for i in range(positions_x.shape[1]):  # Iterate over iterations
    ax.clear()

    ax.scatter(positions_x[:, i], positions_y[:, i], positions_z[:, i],
               color='b', marker='o', label='Actual' if i == 0 else "")  # Circles for actual positions
    ...
    ax.set_zlim([np.min(positions_z), np.max(positions_z)])
    ...
    plt.pause(0.25)  # Adjust pause duration for speed
