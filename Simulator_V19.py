...
for ii in range(iter+N_horizon+1):
    ...
    if ii<=iter:
        xd_pos = np.reshape(np.array([xd_iter[::6], xd_iter[1::6], xd_iter[2::6]]), (3, rows * cols)).reshape(-1, 1,
                                                                                                              order='F')
        [u_shape, u_components_save[:, ii]] = shape_controller_3D_V2(alpha_H, alpha_G, alpha_0,
        ...
        if time_change > ii * delta_factor * T_s:
            sep = iter / n_tasks
            c_0 = np.array([0.0 + 0.3 * ii / sep, 0.0, 0.05 + 0.45 * ii / sep])
...
mpc = init_MPC_model6(x, str_stif,shear_stif,flex_stif,damp_point,damp_quad,l0,n_points, n_points2, n_actuators, x_actuators, mass_points*rows*cols/(n_points*n_points2), mass_quads,Q_vector, R_vector, delta, u_limits, g, xd_sampled, N_horizon)
mpc.setup()
mpc.x0 = x
...
plot_positions(t_save[0:step-1], xe_save[:,0:step-1], xd_save[:,0:step-1], quad_positions, rows, n_actuators)
plot_forces(t_save[0:step-1], u_save[:,0:step-1])
plot_errors(t_save[0:step-1], step-1, xd_save[:,0:step-1], xe_save[:,0:step-1], x_gamma_save[:,0:step-1], n_tasks)
plot_u_errors(t_save[0:step-1], u_components_save[:,0:step-1])
