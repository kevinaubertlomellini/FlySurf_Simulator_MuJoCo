...
spring_factor = 25
# CONTROL PARAMETERS
Q_vector = [180, 110, 0, 0, 5, 5, 0, 0] # [x and y, z, v_x and v_y, v_z, x_UAV and y_UAV, z_UAV , v_x_quad and v_y_quad, v_z_quad]
R_vector = np.array([80000, 15000]) # [force in x and y, force in z]
...
with mujoco.viewer.launch_passive(model, data) as viewer:
    ...
    output = cv2.VideoWriter("LQR_Mujoco_25UAV.avi", cv2.VideoWriter_fourcc(*'MPEG'), 1 / T_s, (view_width, view_height))
    ...
    while viewer.is_running() and data.time <= total_time:
        ...
        if time_num%delta_factor == 0:
            ...
            for kv in range(1, n_actuators + 1):
                u_lqr[3 * kv - 3] = np.clip(u_lqr[3 * kv - 3], u_limits[0, 0], u_limits[0, 1])
                u_lqr[3 * kv - 2] = np.clip(u_lqr[3 * kv - 2], u_limits[1, 0], u_limits[1, 1])
                u_lqr[3 * kv - 1] = np.clip(u_lqr[3 * kv - 1], u_limits[2, 0], u_limits[2, 1])
